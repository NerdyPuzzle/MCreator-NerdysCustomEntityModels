...
@Mod.EventBusSubscriber(bus = Mod.EventBusSubscriber.Bus.MOD) public class ${JavaModName}Variables {
	...
	public static class PlayerVariables {
		...
		public void syncPlayerVariables(Entity entity) {
			...
			${JavaModName}.PACKET_HANDLER.send(PacketDistributor.DIMENSION.with(serverPlayer.level()::dimension), new PlayerVariablesSyncMessage(this, serverPlayer.getId()));
		}
	...
	public static class PlayerVariablesSyncMessage {
		...
		public PlayerVariablesSyncMessage(PlayerVariables data, int player) {
			this.data = data;
			this.player = player;
		}
		...
		public static void handler(PlayerVariablesSyncMessage message, Supplier<NetworkEvent.Context> contextSupplier) {
			...
			context.enqueueWork(() -> {
				if (!context.getDirection().getReceptionSide().isServer()) {
				    Player player = (Player) Minecraft.getInstance().player.level().getEntity(message.player);
					PlayerVariables variables = ((PlayerVariables) player.getCapability(PLAYER_VARIABLES_CAPABILITY, null).orElseGet(PlayerVariables::new));
					<#list variables as var>
