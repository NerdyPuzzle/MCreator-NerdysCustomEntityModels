...
@Mod.EventBusSubscriber(Dist.CLIENT) public class ${name}Procedure {
	...
	public static void renderHumanoid(RenderPlayerEvent playerRenderEvent, PlayerModel model, VertexConsumer vertexConsumer) {
        ...
        poseStack.pushPose();
        playerRenderEvent.getRenderer().setupRotations(eventEntity_, poseStack, ageInTicks, interpolatedBodyYaw, partialTick);
        ...
        model.renderToBuffer(poseStack, vertexConsumer, playerRenderEvent.getPackedLight(), LivingEntityRenderer.getOverlayCoords(eventEntity_, 0), 1, 1, 1, 1);
        offset.negate();
        offsetScale(model, offset);
        poseStack.popPose();
    }
