class FieldModelSelector extends Blockly.Field {
    EDITABLE = true;
    SERIALIZABLE = true;
    CURSOR = 'default';
    ...
    onMouseDown_(e) {
        if (this.sourceBlock_ && !this.sourceBlock_.isInFlyout) {
            if (this.lastClickTime !== -1 && ((new Date().getTime() - this.lastClickTime) < 500)) {
                ...
                modelbridge.openDataListEntrySelector('java_model', {
    ...
    getText_() {
        return this.readableName || FieldModelSelector.getDefaultText();
    }
    ...
    updateReadableName(value) {
        if (this.cachedReadableName) {
            ...
            this.readableName = FieldModelSelector.getDefaultText();
        }
    }
}

Blockly.fieldRegistry.register('field_model_selector', FieldModelSelector);