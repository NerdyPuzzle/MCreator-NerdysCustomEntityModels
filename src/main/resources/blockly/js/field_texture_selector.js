class FieldTextureSelector extends Blockly.Field {
    EDITABLE = true;
    SERIALIZABLE = true;
    CURSOR = 'default';
    ...
    onMouseDown_(e) {
        if (this.sourceBlock_ && !this.sourceBlock_.isInFlyout) {
            if (this.lastClickTime !== -1 && ((new Date().getTime() - this.lastClickTime) < 500)) {
                ...
                modelbridge.openDataListEntrySelector('entity_texture', {
    ...
    getText_() {
        return this.readableName || FieldTextureSelector.getDefaultText();
    }
    ...
    updateReadableName(value) {
        if (this.cachedReadableName) {
            ...
            this.readableName = FieldTextureSelector.getDefaultText();
        }
    }
}
