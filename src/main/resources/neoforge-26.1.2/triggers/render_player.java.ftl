...
@EventBusSubscriber(Dist.CLIENT) public class ${name}Procedure {
	...
	public static void renderEntity(RenderPlayerEvent playerRenderEvent, EntityModel model, RenderType renderType, LivingEntityRenderState state) {
        LivingEntity eventEntity_ = (LivingEntity) playerRenderEvent.getRenderState().getRenderData(${JavaModName}RenderStateModifiers.LIVING_ENTITY);
        PoseStack poseStack = playerRenderEvent.getPoseStack();
        poseStack.pushPose();
		if (eventEntity_.hasPose(Pose.SLEEPING)) {
		...
		playerRenderEvent.getRenderer().setupRotations((AvatarRenderState) state, poseStack, state.bodyRot, 69);
		poseStack.scale(-0.938f, -0.938f, 0.938f);
		poseStack.translate(0.0D, -1.501, 0.0D);
        model.setupAnim(state);
        ...
        if (model instanceof ${JavaModName}AnimatedModels.Animatable animatable) {
    		...
    		playerModel.setupAnim((AvatarRenderState) state);
            if (playerData.contains("PlayerAnimationProgress") && playerData.getFloatOr("PlayerAnimationProgress", 0) > 0) {
                playerData.putFloat("PlayerAnimationProgress", oldAnimationProgress);
                playerData.putFloat("LastTickTime", oldAgeInTicks);
            }
